...
class CreateEvent extends React.Component {
  ...
  onSelectCoverImage = debounce(
    () => {
      ...
      const evenPairsCount = images.length / 2;

      for (var i = 0; i < evenPairsCount; i++) {
        const action = images.splice(0, 2);
        const objFromAction = {
          image1: action[0] || null,
          image2: action[1] || null,
        };
        photos.push(objFromAction);
      }
  ...
  _renderItem = ({item: {image1, image2}, index}) => {
    console.log('Images >>>>', this.state.photos);
    if (index === 0) {
      return (
        <View>
          <AppButton
          ...
          />
          {this._clickableImage(image2)}
        </View>
      );
    }
    return (
      <View>
        {this._clickableImage(image1)}
        {image2 && this._clickableImage(image2)}
      </View>
    );
  };
...
const styles = StyleSheet.create({
  ...
  cover_image_list: {
    ...
    height: getDimentions().height / 10 - 5,
  },
